  ...
  class Sensei
    ...
    def observe(step)
      if step.passed?
        ...
        if @pass_count > progress.last.to_i
          @observations << Color.green("#{step.koan_file}: #{step.name} test passed.")
        end
      else
        @failed_test = step
        @failure = step.failure
        add_progress(@pass_count)
        @observations << Color.red("#{step.koan_file}: #{step.name} test failed.")
        throw :neo_exit
      end
    end
    ...
    def artistic_end_screen
      ...
      ruby_version = ruby_version.side_padding(54)
        puts Color.green("All tests successfully passed")
    end
